...
```Tokens are used as a way to get the server to trust us. Basically proving that we are who we say we are ```
- [x] What steps can you take in your web apps to keep your data secure?
- [x] Describe how web servers work.
```Servers are our files or data that is located in a REMOTE computer where is being hosted by a provider.```
- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
``` These methods are GET, PUT, DELETE, POST```
